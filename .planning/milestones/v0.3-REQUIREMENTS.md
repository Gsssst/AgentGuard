# Requirements Archive: v0.3 LangGraph Adapter
...
- [x] **BATCH-01**: `GuardedToolNode` can process multiple `tool_calls` from one `AIMessage`.
- [x] **BATCH-02**: Independent calls can execute concurrently while conflicting calls obey AgentGuard resource-lock semantics.
- [x] **BATCH-03**: Failure of one call does not automatically cancel other independent calls.
- [x] **BATCH-04**: Returned `ToolMessage` objects preserve input call order and each original call ID.
- [x] **BATCH-05**: Permission denial, timeout, retry exhaustion, lock timeout, and unknown-tool outcomes are represented as structured messages.

### Approval Bridge

- [x] **APPROVAL-01**: Approval-required tool calls pause through LangGraph `interrupt` before the underlying tool is invoked.
- [x] **APPROVAL-02**: The interrupt payload contains only redacted tool information, an argument summary, the `tool_call_id`, and an action digest.
- [x] **APPROVAL-03**: Approval resumes through `Command(resume=...)`, with LangGraph owning checkpoint persistence and graph recovery state.
- [x] **APPROVAL-04**: A reviewer can approve or reject each pending call independently by `tool_call_id`.
- [x] **APPROVAL-05**: Resume recomputes the action digest and refuses execution when the call arguments no longer match the approved action.
- [x] **APPROVAL-06**: Only approved calls execute; rejected calls return structured `ToolMessage` results.
...
- [x] **COMPAT-04**: Automated tests cover success, denial, timeout, retry exhaustion, lock conflict, approval, and digest-mismatch scenarios.
...
| COMPAT-04 | Phase 9/10 | Complete |
| BATCH-01..05 | Phase 8/10 | Complete |
| APPROVAL-01..06 | Phase 9/10 | Complete |
...
- **Validated:** 22/22 requirements.
- **Adjusted:** BATCH-04, APPROVAL-03, and APPROVAL-06 received an additional
  closure phase after the milestone audit exposed the standard
  `MessagesState + add_messages` duplication bug.
- **Dropped:** None.

---
*Archived: 2026-09-04 after v0.3 milestone verification*
